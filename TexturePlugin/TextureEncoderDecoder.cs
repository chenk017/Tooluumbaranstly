...
namespace TexturePlugin
{
    public class TextureEncoderDecoder
    {
        ...
        //
        // Fallback:
        //     RGBADecoders.ReadRGB565()
        //
        // RGB565 diperlukan karena hasil pengujian menunjukkan:
        //
        //     DecodeManaged(RGB565) -> NULL
        //     ReadRGB565()          -> valid RGBA
        //
        // ============================================================
            ...
            TextureFormat format)
        {
            ...
            // --------------------------------------------------------
            // 3. Pastikan decoder menghasilkan RGBA
            // --------------------------------------------------------
            ...
            // 4. R/B swap
            //
            // Ini adalah behavior asli yang sudah terbukti
            // menghasilkan PNG benar pada texture test.
            // --------------------------------------------------------
        ...
        private static byte[] DecodePVRTexLib(
            byte[] data,
            int width,
            int height,
            TextureFormat format)
        {
            ...
            unsafe
            {
                ...
                fixed (byte* destPtr = dest)
                {
                    ...
                    size = PInvoke.DecodeByPVRTexLib(
                        dataIntPtr,
                        destIntPtr,
                        (int)format,
                        (uint)width,
        ...
        private static byte[] DecodeCrunch(
            byte[] data,
            int width,
            int height,
            TextureFormat format)
        {
            ...
            unsafe
            {
                ...
                fixed (byte* destPtr = dest)
                {
                    ...
                    size = PInvoke.DecodeByCrunchUnity(
                        dataIntPtr,
                        destIntPtr,
                        (int)format,
                        (uint)width,
        ...
        private static byte[] EncodeISPC(
            byte[] data,
            int width,
            int height,
            TextureFormat format,
            int quality)
        {
            ...
            {
                throw new Exception(
                    $"ispc ({format}) encoded more data than expected!");
            }
        ...
        private static byte[] EncodePVRTexLib(
            byte[] data,
            int width,
            int height,
            TextureFormat format,
            int quality)
        {
            ...
            unsafe
            {
                ...
                fixed (byte* destPtr = dest)
                {
                    ...
                    size = PInvoke.EncodeByPVRTexLib(
                        dataIntPtr,
                        destIntPtr,
                        (int)format,
                        quality,
            ...
            {
                throw new Exception(
                    $"pvrtexlib ({format}) encoded more data than expected!");
            }
            else if (size == expectedSize)
            {
                return dest;
        ...
        private static byte[] EncodeCrunch(
            byte[] data,
            int width,
            int height,
            TextureFormat format,
            ...
            int mips)
        {
            ...
            unsafe
            {
                ...
                fixed (byte* dataPtr = data)
                {
                    ...
                    // write it to unmanaged memory first and copy to
                    // managed after we have the size

                    IntPtr dataIntPtr = (IntPtr)dataPtr;

                    size = PInvoke.EncodeByCrunchUnity(
                        dataIntPtr,
                        ref checkoutId,
                        (int)format,
                        quality,
            ...
            TextureFormat format)
        {
            switch (format)
            {
                ...
                // PVRTEXLIB
                //
                // RGB565 SUDAH DIKELUARKAN DARI SINI.
                // ====================================================
                ...
                // ----------------------------------------------------
                // NEW MANAGED FALLBACK
                // ----------------------------------------------------
                case TextureFormat.RGB565:
                {
                    byte[] res =
                        DecodeAssetRipperTex(
                            data,
                            width,
                            height,
                            format);
        ...
        public static byte[] Encode(
            ...
            int mips = 1)
        {
            using MemoryStream rawDataStream =
                new MemoryStream();
